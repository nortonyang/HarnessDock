    ...
    *,
    ...
    if is_relative_to(source, run_dir):
        raise SystemExit(
            ...
            "$imagegen output from $CODEX_HOME/generated_images/.../ig_*.png or exec-*.png instead"
        )
    generated_root = default_generated_images_root()
    if not is_relative_to(source, generated_root) or not source.name.startswith(("ig_", "exec-")):
        raise SystemExit(
            "source image does not look like a built-in $imagegen output; expected "
            f"{generated_root}/.../ig_*.png or exec-*.png. Do not ingest locally drawn or "
