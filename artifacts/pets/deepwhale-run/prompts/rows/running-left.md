Create a single horizontal sprite strip for the Codex app digital pet `deepwhale` in the state `running-left`.
...
Repair attempt 1:
- The previous `running-left` strip failed QA: must be re-derived after the corrected running-right strip is approved
