...
- App price: Free. HarnessDock source code is available under the MIT License; DeepSeek API usage is billed separately by DeepSeek.
...
Free and open source under the MIT License. Users bring their own DeepSeek API access; any API charges are paid directly to DeepSeek.
...
免费，并采用 MIT License 开源。用户自行提供 DeepSeek API，相关 API 费用直接由 DeepSeek 收取。
...
- [ ] Replace the current SF Symbols-derived app icon before using it as a launch thumbnail.
- [ ] Export and verify a new 240×240 Product Hunt thumbnail from an original, authorized icon.
- [x] Added a sanitized overview screenshot with private workspace and account details redacted.
