...
当前公开源码已可供开发者审阅和自行构建，但不建议把现有 `dist/HarnessDock.app` 作为面向普通用户的正式安装包发布。代码中未发现硬编码 API Key 或私钥，项目已采用 MIT License，GitHub 首页也已使用脱敏截图；仍有图标授权、软件签名、公证、发布架构和完整画廊等阻塞项。
...
| GitHub 首页截图包含私人信息 | 已修复 | 首页改用 `docs/screenshots/sanitized/harnessdock-overview-redacted.png`，工作区、模式、模型、输入内容和余额均已遮盖 |
...
1. 替换 AppIcon，并使用无私人路径、无真实会话的演示环境扩充脱敏截图画廊。
2. 已添加 MIT License；发布安装包前继续核对需要随包分发的第三方 notices，并建立 `SECURITY.md`。
