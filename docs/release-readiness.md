...
| 余额专用 `DEEPSEEK_API_KEY` 被 npm/Harness/插件继承 | 已修复 | `AppModel` 在创建子进程环境时移除该变量 |
...
| P1 | `npx --yes @deepseek-ai/dsh` 未锁版本 | 首次启动会执行当时的最新包；上游预览版存在破坏性变化，供应链结果不可复现 | 固定经过验证的确切版本，升级改为显式、可回滚流程 |
...
| P2 | 当前没有自动更新、漏洞响应或固定依赖清单 | 用户可能长期停留在有风险的旧版，或无法判断运行的上游版本 | 显示版本、记录上游版本与校验值，建立 `SECURITY.md` 和发布/撤回流程 |
...
3. 锁定并显示已验证的 `@deepseek-ai/dsh` 版本，完成从干净用户环境安装的端到端测试。
