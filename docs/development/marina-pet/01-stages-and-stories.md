...
| ST-609 | 三边藏身探头姿态优化 | US-612 | 已完成 |
| ST-610 | 交互动作语义与节奏修复 | US-613 | 已完成 |
...
作为把宠物放在窗口边框的用户，我希望看到它把身体藏在窗口外、只把头探进来，而不是整张精灵被垂直或水平切成两半。

验收标准：

- Given 宠物停靠左侧或右侧并处于缩回状态，When 查看轮廓，Then 身体比头部藏得更深，头部向内容区倾斜，左右姿态互为语义镜像。
- Given 光标进入、点击或自动探头，When 宠物探出，Then 位移向内容区推进且倾角减小，呈现从躲藏到探出的连续动作。
- Given 宠物停靠底部，When 缩回与探出，Then 缩回态主要保留头部、隐藏下半身，探出态显示更多身体。
- Given 用户拖动宠物，When 进入拖动状态，Then 取消藏身倾角但保留左侧朝向，命中区域与 `pointer-events: none` 契约不变。

## US-613：交互动作自然且不串行

作为与宠物互动的用户，我希望悬停、点击和拖动分别表现为观察、跳跃和奔跑，而不是失败、发呆或突然消失。

验收标准：

- Given 光标进入宠物区域，When 只悬停不按下，Then 播放 waiting 第 6 行共 6 帧，不进入 failed 第 5 行。
- Given 用户点击宠物，When 播放一次反馈，Then 播放 jumping 第 4 行共 5 帧，循环中不存在第 6 个透明空格。
- Given 用户拖动宠物越过阈值，When 宠物跟随指针，Then 播放 running 第 7 行共 6 帧，松手后过渡到挥手再回待机。
- Given 用户连续悬停、点击或拖动，When 状态互相切换，Then 新状态从第 0 帧开始，节奏与位移过渡连续，按钮穿透和贴边偏好不回归。
