...
| 3 | 创建正式插件包 | `plugins/harnessdock-pet` | manifest/patch 静态检查 | 已完成 |
| 4 | 构建单文件 client bundle，内嵌 DeepWhale / Marina 图集 | `plugins/harnessdock-pet/lib/client.js` | data URL、哈希、bundle wrapper 检查 | 已完成 |
...
| 7 | 回归 HarnessDock 原生构建与更新说明 | README、checks、dist | checks/build | 已完成 |
| 8 | 扩展偏好模型，保存 `edge` 与沿边 `offset` | `plugins/harnessdock-pet/src/client.template.js` | 非法旧值迁移与纯函数检查 | 已完成 |
...
| 15 | 安装 app、重绑 profile 并重启 Harness | `/Applications/HarnessDock.app`、web profile | link 目标、3080 启动图、真实 UI | 已完成 |
...
- 包名 `@harnessdock/pet`，Loader ID `pet`。
...
- 设置使用 `harnessdock.pet.preferences.v1` localStorage 键；从旧 `dsharness.pet.preferences.v1` 无感迁移，并仅接受白名单宠物 ID 和布尔值。
...
- 如果 Harness 插槽契约升级，插件在包级隔离，可更新 bundle 而不迁移 HarnessDock 主应用数据。
