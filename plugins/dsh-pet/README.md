...
当前 DeepSeek Harness 官方 CLI 支持从 npm、GitHub 和本地路径安装插件，但公开资料中没有官方插件网站的上传入口。此包可先发布到 GitHub，或在把 `@dsharness` 替换为发布者拥有的 npm scope 后发布到 npm；希望成为官方包时，应向 [`deepseek-ai/deepseek-harness`](https://github.com/deepseek-ai/deepseek-harness) 提交提案或 PR，由上游维护者审核收录。

发布前预检：

```bash
npm pack ./plugins/dsh-pet --dry-run
```
